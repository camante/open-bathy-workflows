...
from legacy.river.river_v2_contract import RiverV2StageResult, STAGE_CENTERLINE_OFFSET_MODELED
...
def centerline_modeled_offset_field_schema() -> dict[str, str]:
    return {
        ...
        "offset_prior_basis": "string",
        "offset_prior_component_id": "string",
...
def _read_transfer_priors(path: Path) -> gpd.GeoDataFrame:
    if not path.exists():
        raise RuntimeError(f"river_v2_modeled_offset_missing_transfer_priors:{path}")
    priors = gpd.read_file(path)
    if "component_id" not in priors.columns:
        raise RuntimeError("river_v2_modeled_offset_transfer_priors_missing_component_id")
    priors["component_id"] = priors["component_id"].astype(str)
    return priors


def _lookup_transfer_prior(priors: gpd.GeoDataFrame, group: gpd.GeoDataFrame) -> pd.Series | None:
    if len(priors) == 0 or len(group) == 0 or "component_id" not in group.columns:
        return None
    comp = str(group.iloc[0].get("component_id", "") or "")
    hit = priors[priors["component_id"].astype(str) == comp]
    if hit.empty:
        return None
    return hit.iloc[0]


def _build_global_scalar_modeled_offsets(joined: gpd.GeoDataFrame) -> tuple[gpd.GeoDataFrame, gpd.GeoDataFrame, list[str], dict[str, Any]]:
    out = joined.copy()
    ...
    observed = np.where(np.isfinite(observed), np.maximum(observed, _MIN_OFFSET_M), np.nan)
    anchor_mask = np.isfinite(observed)
    if int(np.count_nonzero(anchor_mask)) == 0:
        raise RuntimeError("river_v2_modeled_offset_global_scalar_no_observed_offsets")
    scalar_offset_m = float(np.nanmedian(observed[anchor_mask]))
    modeled = np.full((len(out),), scalar_offset_m, dtype=float)
    modeled[anchor_mask] = observed[anchor_mask]
    out["observed_offset_m"] = observed
    out["offset_modeled_m"] = modeled
    out["offset_source"] = np.where(anchor_mask, "observed_offset_anchor", "solve_domain_global_observed_scalar")
    out["offset_confidence"] = np.where(anchor_mask, "high", "medium")
    out["offset_support_class"] = np.where(anchor_mask, "observed_anchor", "inferred")
    out["offset_prior_basis"] = np.where(anchor_mask, "local_observed_component", "solve_domain_global_observed_scalar")
    out["offset_prior_component_id"] = ""
    out["placeholder_basis"] = ""
    ...
    keep = [c for c in ("point_id", "station_m", *_GROUP_COLS, "observed_offset_m", "offset_modeled_m", "offset_source", "offset_confidence", "offset_support_class", "offset_prior_basis", "offset_prior_component_id", "placeholder_basis", "stream_order", "mean_width_m", "mean_slope", "geometry") if c in out.columns]
    out = gpd.GeoDataFrame(out[keep].copy(), geometry="geometry", crs=joined.crs)
    sort_cols = [c for c in (*_group_columns(out), "station_m", "point_id") if c in out.columns]
    ...
    if len(component_summary):
        ...
        component_summary["placeholder_offset_m"] = np.nan
        component_summary["placeholder_basis"] = ""
        component_summary["placeholder_confidence"] = ""
    diagnostics = {
        "record_count": int(len(out)),
        "group_count": int(len(component_summary)),
        ...
        "global_scalar_offset_m": scalar_offset_m,
        "global_scalar_anchor_count": int(np.count_nonzero(anchor_mask)),
        "global_scalar_modeled_point_count": int(len(out) - np.count_nonzero(anchor_mask)),
        "inferred_count": int(np.count_nonzero(out["offset_support_class"].astype(str) == "inferred")),
        "unsupported_component_count": 0,
        "placeholder_modeled_point_count": 0,
        "transferred_prior_modeled_point_count": 0,
        ...
        "modeled_offset_strategy": "solve_domain_global_observed_scalar",
    }
    warnings = ["modeled_offset_used_global_scalar_from_solve_domain_observed_offsets"]
    return out, component_summary, warnings, diagnostics


def _modeled_series_from_transfer_prior(station: np.ndarray, prior_row: pd.Series) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    sta = np.asarray(station, dtype=float)
    finite_station = np.isfinite(sta)
    modeled = np.full((sta.size,), np.nan, dtype=float)
    source = np.full((sta.size,), "unsupported_component_no_modeled_offset", dtype=object)
    ...
    if not np.any(finite_station):
        return modeled, support, source
    up = pd.to_numeric(prior_row.get("prior_upstream_offset_m"), errors="coerce")
    down = pd.to_numeric(prior_row.get("prior_downstream_offset_m"), errors="coerce")
    median = pd.to_numeric(prior_row.get("prior_median_offset_m"), errors="coerce")
    slope = pd.to_numeric(prior_row.get("prior_slope_m_per_m"), errors="coerce")
    if not np.isfinite(up) and np.isfinite(median):
        up = float(median)
    if not np.isfinite(down) and np.isfinite(median):
        down = float(median)
    if not np.isfinite(up) and np.isfinite(down):
        up = float(down)
    if not np.isfinite(down) and np.isfinite(up):
        down = float(up)
    if np.isfinite(up) and np.isfinite(down):
        sta_f = sta[finite_station]
        s0 = float(np.nanmin(sta_f))
        s1 = float(np.nanmax(sta_f))
        if abs(s1 - s0) > 1.0e-9:
            modeled[finite_station] = np.interp(sta_f, np.asarray([s0, s1], dtype=float), np.asarray([float(up), float(down)], dtype=float))
        else:
            modeled[finite_station] = float(np.nanmean([up, down]))
    elif np.isfinite(median):
        modeled[finite_station] = float(median)
    elif np.isfinite(slope):
        sta_f = sta[finite_station]
        modeled[finite_station] = float(_MIN_OFFSET_M) + float(slope) * (sta_f - float(np.nanmin(sta_f)))
    source[finite_station] = "transferred_component_prior"
    support[finite_station] = "inferred"
    return modeled, support, source


def _build_modeled_offsets_for_group(group: gpd.GeoDataFrame, transfer_prior_row: pd.Series | None = None) -> tuple[gpd.GeoDataFrame, dict[str, Any], list[str]]:
    ...
    if anchor_count >= 1:
        ...
        offset_prior_basis = np.full((len(out),), "local_observed_component", dtype=object)
        offset_prior_component_id = np.full((len(out),), str(transfer_prior_row.get("prior_source_component_id", "")) if transfer_prior_row is not None else str(out.iloc[0].get("component_id", "") if len(out) else ""), dtype=object)
    else:
        warnings.append("modeled_offset_component_has_no_anchor_support")
        if transfer_prior_row is not None and pd.notna(transfer_prior_row.get("prior_median_offset_m")):
            modeled, support_class, source = _modeled_series_from_transfer_prior(station, transfer_prior_row)
            offset_confidence = np.full((len(out),), str(transfer_prior_row.get("prior_confidence", "medium") or "medium"), dtype=object)
            placeholder_basis = np.full((len(out),), "", dtype=object)
            offset_prior_basis = np.full((len(out),), str(transfer_prior_row.get("prior_basis", "") or ""), dtype=object)
            offset_prior_component_id = np.full((len(out),), str(transfer_prior_row.get("prior_source_component_id", "") or ""), dtype=object)
        else:
            ...
            offset_prior_basis = np.full((len(out),), "", dtype=object)
            offset_prior_component_id = np.full((len(out),), "", dtype=object)
    ...
    out["offset_prior_basis"] = offset_prior_basis.astype(str)
    out["offset_prior_component_id"] = offset_prior_component_id.astype(str)
    ...
    diagnostics = {
        ...
        "transferred_prior_modeled_point_count": int(np.count_nonzero(out["offset_source"].astype(str) == "transferred_component_prior")),
    }
    return out, diagnostics, warnings


def build_modeled_offsets(joined: gpd.GeoDataFrame, transfer_priors: gpd.GeoDataFrame | None = None) -> tuple[gpd.GeoDataFrame, gpd.GeoDataFrame, list[str], dict[str, Any]]:
    if transfer_priors is None:
        transfer_priors = gpd.GeoDataFrame({"component_id": pd.Series(dtype="object")})
    component_summary = _build_component_stream_summary(joined)
    group_cols = _group_columns(joined)
    ...
    for _, group in group_iter:
        modeled_group, group_diag, group_warnings = _build_modeled_offsets_for_group(group, _lookup_transfer_prior(transfer_priors, group))
    ...
    diagnostics = {
        ...
        "transferred_prior_modeled_point_count": int(sum(d.get("transferred_prior_modeled_point_count", 0) for d in diagnostics_list)),
        "supported_component_count": int(np.count_nonzero(pd.to_numeric(component_summary.get("observed_offset_count"), errors="coerce").to_numpy(dtype=float) > 0.0)) if len(component_summary) else 0,
        "unsupported_component_count_from_summary": int(np.count_nonzero(pd.to_numeric(component_summary.get("observed_offset_count"), errors="coerce").to_numpy(dtype=float) <= 0.0)) if len(component_summary) else 0,
    }
...
def run_modeled_offset_stage(
    ...
    *,
    ...
    transfer_priors_path: Path,
) -> RiverV2StageResult:
    joined = _join_inputs(wse_points_path, observed_offset_points_path)
    use_global_scalar = bool(
        (not bool(getattr(ctx, "local_authoritative_control_found", False)))
        and bool(getattr(ctx, "downstream_authoritative_control_found", False))
        and bool(np.count_nonzero(np.isfinite(_numeric(joined.get("observed_offset_m", pd.Series(dtype=float))))))
    )
    if use_global_scalar:
        modeled, component_summary, warnings, diagnostics = _build_global_scalar_modeled_offsets(joined)
    else:
        transfer_priors = _read_transfer_priors(transfer_priors_path)
        modeled, component_summary, warnings, diagnostics = build_modeled_offsets(joined, transfer_priors)
    ...
    component_summary_receipt = build_river_v2_stage_receipt(
        ...
        input_artifacts=ctx.direct_stage_input_artifacts(wse_points_path, observed_offset_points_path, transfer_priors_path),
    ...
    receipt = build_river_v2_stage_receipt(
        ...
        warnings=warnings,
        source_logic=(
            "modeled_offset_from_single_solve_domain_observed_scalar_when_export_aoi_has_no_local_authoritative_bed_support"
            if diagnostics.get("modeled_offset_strategy") == "solve_domain_global_observed_scalar"
            else "modeled_offset_from_local_observed_offsets_first_transferred_component_priors_second_explicit_nhdplus_placeholder_last_never_dem_bed_proxy"
        ),
