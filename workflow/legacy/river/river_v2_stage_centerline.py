...
from legacy.river.river_v2_contract import RiverV2StageResult, STAGE_RIVER_CENTERLINE
...
def _filter_points_to_domain_mask(gdf: gpd.GeoDataFrame, domain_mask_path: Path | None) -> tuple[gpd.GeoDataFrame, dict[str, Any]]:
    if gdf is None or gdf.empty:
        return gdf, {"applied": False, "reason": "empty_centerline", "input_record_count": int(len(gdf) if gdf is not None else 0), "kept_record_count": int(len(gdf) if gdf is not None else 0)}
    if domain_mask_path is None or not Path(domain_mask_path).exists():
        return gdf, {"applied": False, "reason": "missing_domain_mask", "input_record_count": int(len(gdf)), "kept_record_count": int(len(gdf))}

    work = gdf.copy()
    try:
        with rasterio.open(domain_mask_path) as ds:
            target_crs = ds.crs
            if target_crs is not None and work.crs is not None and str(work.crs) != str(target_crs):
                ...
                if geom is not None and not geom.is_empty:
                    ...
                    try:
                        ...
                        if 0 <= row < ds.height and 0 <= col < ds.width:
                            on_grid_count += 1
                            keep = bool(valid[row, col])
                    except Exception:
                        keep = False
                kept.append(bool(keep))
            keep_arr = np.asarray(kept, dtype=bool)
            filtered = work.loc[keep_arr].copy()
            centerline_bounds = tuple(float(v) for v in work.total_bounds.tolist()) if len(work) else None
            mask_bounds = tuple(float(v) for v in ds.bounds)
            return filtered, {
                "applied": True,
                "reason": "domain_mask_filter",
                "domain_mask_path": str(domain_mask_path),
                "target_crs": str(target_crs) if target_crs is not None else None,
                "centerline_crs": str(work.crs) if work.crs is not None else None,
                "centerline_bounds": centerline_bounds,
                "mask_bounds": mask_bounds,
                "input_record_count": int(len(work)),
                "on_grid_record_count": int(on_grid_count),
                "kept_record_count": int(len(filtered)),
                "dropped_record_count": int(len(work) - len(filtered)),
            }
    except Exception as exc:
        return work, {"applied": False, "reason": f"domain_mask_filter_failed:{type(exc).__name__}", "input_record_count": int(len(work)), "kept_record_count": int(len(work))}
...
def run_centerline_stage(ctx: RiverV2Context, *, network_gpkg: str | Path | None, river_dem_path: str | Path | None, sampling_raster_path: str | Path | None, channel_mask_path: str | Path | None, spacing_m: float, min_stream_order: int) -> RiverV2StageResult:
    paths = ctx.paths
    ...
    domain_mask_path = Path(channel_mask_path) if channel_mask_path is not None else None
    ...
    export_centerline_path = None
    export_filter_summary: dict[str, Any] | None = None
    export_centerline_source: str | None = None
    export_mask_path = Path(ctx.export_channel_mask_path) if getattr(ctx, "export_channel_mask_path", None) is not None else None
    if export_mask_path is not None and domain_mask_path is not None and str(export_mask_path) != str(domain_mask_path):
        export_network_path = Path(getattr(ctx, "export_network_gpkg", network_path)) if getattr(ctx, "export_network_gpkg", None) is not None else network_path
        gdf_export_source = gdf_all
        export_centerline_source = "solve_network_centerline_points"
        if export_network_path is not None and export_network_path.exists() and (network_path is None or str(export_network_path) != str(network_path)):
            try:
                gdf_export_source = _derive_centerline_points(
                    inline_centerline_points_gdf=None,
                    provided_source=None,
                    network_gpkg=export_network_path,
                    river_dem_path=dem_path,
                    sampling_raster_path=sampling_path,
                    spacing_m=float(spacing_m),
                    min_stream_order=int(min_stream_order),
                )
                export_centerline_source = "export_network_centerline_points"
            except Exception as exc:
                export_centerline_source = f"solve_network_centerline_points_after_export_network_failure:{type(exc).__name__}"
        gdf_export, export_filter_summary = _filter_points_to_domain_mask(gdf_export_source, export_mask_path)
        if not gdf_export.empty:
            export_centerline_path = write_centerline_points_gpkg(gdf_export, paths.river_centerline_points_export, slim=True)
    gdf_domain, domain_filter_summary = _filter_points_to_domain_mask(gdf_all, domain_mask_path)
    if gdf_domain.empty:
        raise RuntimeError(f"river_v2_centerline_no_points_in_domain:{domain_filter_summary}")
    gdf, filter_summary = _filter_centerline_components_simple(gdf_domain, sampling_path)
    ...
    receipt = build_river_v2_stage_receipt(
        ...
        input_artifacts=ctx.direct_stage_input_artifacts(network_path, dem_path, sampling_path, domain_mask_path),
        ...
        validation={**validation, "domain_filter": domain_filter_summary, "component_filter": filter_summary},
    )
    ...
    return RiverV2StageResult(
        ...
        record_count=int(len(gdf)),
        validation={**validation, "domain_filter": domain_filter_summary, "component_filter": filter_summary, "export_filter": export_filter_summary, "export_centerline_source": export_centerline_source},
        warnings=[],
        aux_outputs={
            "river_centerline_points_all": str(all_path),
            **({"river_centerline_points_export": str(export_centerline_path)} if export_centerline_path is not None else {}),
        },
    )
