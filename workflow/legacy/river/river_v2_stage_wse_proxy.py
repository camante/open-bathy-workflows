...
from legacy.river.river_v2_contract import RiverV2StageResult, STAGE_CENTERLINE_WSE_PROXY
...
def _sample_raster_nearest_finite_support(
    gdf: gpd.GeoDataFrame,
    raster_path: Path,
    *,
    ...
    with rasterio.open(raster_path) as ds:
        ...
        if rows.size == 0:
            return pd.DataFrame({
                "nearest_distance_m": np.full((len(gdf),), np.nan),
                "wse_raw_nearest_z_m": np.full((len(gdf),), np.nan),
            })
        xs, ys = rasterio.transform.xy(ds.transform, rows, cols, offset="center")
        edge_xy = np.column_stack([np.asarray(xs, dtype=float), np.asarray(ys, dtype=float)])
        edge_vals = arr[rows, cols].astype(float)
        ...
        if raster_crs is not None:
            try:
                ...
                pass
        tree = cKDTree(metric_edge_xy)
        dist, idx = tree.query(metric_station_xy, k=1)
        dist = np.asarray(dist, dtype=float)
        idx = np.asarray(idx, dtype=int)
        out = np.full((len(gdf),), np.nan, dtype=float)
        valid = np.isfinite(dist)
        if max_distance_m is not None:
            valid &= dist <= float(max_distance_m)
        if np.any(valid):
            out[valid] = edge_vals[idx[valid]]
        return pd.DataFrame({
            "nearest_distance_m": dist,
            "wse_raw_nearest_z_m": out,
        })
...
def build_wse_support(
    ...
    *,
    bank_wse_edge_guidance_path: Path,
) -> tuple[gpd.GeoDataFrame, Path, dict[str, Any]]:
    edge_path = Path(bank_wse_edge_guidance_path)
    ...
    nearest_support = _sample_raster_nearest_finite_support(work, edge_path, max_distance_m=None)
    direct_support = pd.to_numeric(nearest_support.get("wse_raw_nearest_z_m"), errors="coerce").to_numpy(dtype=float)
    support_distance_m = pd.to_numeric(nearest_support.get("nearest_distance_m"), errors="coerce").to_numpy(dtype=float)
    support_type = np.full((n,), "no_support", dtype=object)
    finite_support = np.isfinite(direct_support)
    direct_hits = finite_support & np.isfinite(support_distance_m) & (support_distance_m <= 36.0)
    nearest_hits = finite_support & ~direct_hits
    support_type[direct_hits] = "direct_local_support"
    support_type[nearest_hits] = "nearest_bank_guidance_support"
    ...
    for _, grp in group_iter:
        ...
        work.loc[idx, "station_direction"] = _resolve_component_station_direction(station[idx], direct_support[idx], support_type[idx])

    work["wse_support_z_m"] = direct_support
    work["support_distance_m"] = support_distance_m
    work["support_type"] = pd.Series(support_type, dtype="object")
    work["has_support"] = np.isfinite(direct_support)

    keep = [c for c in [*_identity_columns(work), "wse_support_z_m", "support_distance_m", "support_type", "has_support"] if c in work.columns]
    ...
    supported_count = int(np.count_nonzero(support_gdf["has_support"].to_numpy(dtype=bool)))
    distance_vals = pd.to_numeric(support_gdf.get("support_distance_m"), errors="coerce").to_numpy(dtype=float)
    finite_distance = distance_vals[np.isfinite(distance_vals)]
    summary = {
        "supported_station_count": supported_count,
        "unsupported_station_count": int(len(support_gdf) - supported_count),
        "direct_local_support_count": int(np.count_nonzero(support_type_vals == "direct_local_support")),
        "nearest_bank_guidance_support_count": int(np.count_nonzero(support_type_vals == "nearest_bank_guidance_support")),
        "selected_bank_wse_edge_guidance_path": str(edge_path),
        "median_support_distance_m": float(np.nanmedian(finite_distance)) if finite_distance.size else None,
        "max_support_distance_m": float(np.nanmax(finite_distance)) if finite_distance.size else None,
    }
    if supported_count == 0:
        summary["warning"] = "wse_support_has_no_finite_values"
    elif supported_count < len(support_gdf):
        summary["warning"] = "wse_support_incomplete_after_nearest_bank_guidance_completion"
    return support_gdf, support_path, summary




def _summarize_support_points(support_gdf: gpd.GeoDataFrame) -> dict[str, Any]:
    vals = pd.to_numeric(support_gdf.get("wse_support_z_m"), errors="coerce").to_numpy(dtype=float) if "wse_support_z_m" in support_gdf.columns else np.full((len(support_gdf),), np.nan)
    types = support_gdf["support_type"].astype(str).to_numpy(dtype=object) if "support_type" in support_gdf.columns else np.full((len(support_gdf),), "unknown", dtype=object)
    supported = int(np.count_nonzero(np.isfinite(vals)))
    summary = {
        "supported_station_count": supported,
        "unsupported_station_count": int(len(support_gdf) - supported),
        "direct_local_support_count": int(np.count_nonzero(types == "direct_local_support")),
        "nearest_bank_guidance_support_count": int(np.count_nonzero(types == "nearest_bank_guidance_support")),
    }
    if supported == 0:
        summary["warning"] = "wse_support_has_no_finite_values"
    elif supported < len(support_gdf):
        summary["warning"] = "wse_support_incomplete_after_nearest_bank_guidance_completion"
    return summary
...
def build_wse_trend(
    ...
    keep = [c for c in [*_identity_columns(work), "wse_support_z_m", "support_distance_m", "support_type", "has_support", "wse_anchor_raw_z_m", "wse_interp_z_m", "wse_trend_z_m"] if c in work.columns]
...
def build_wse_pre_smooth(
    ...
    keep = [c for c in [*_identity_columns(work), "wse_support_z_m", "support_distance_m", "support_type", "has_support", "wse_anchor_raw_z_m", "wse_interp_z_m", "wse_trend_z_m", "wse_pre_smooth_z_m"] if c in work.columns]
...
def build_wse_proxy_final(
    ...
    direct_profile_fallback_count = 0
    group_fields = _group_keys(work)
    group_iter = [(None, work)] if not group_fields else work.groupby(group_fields, dropna=False, sort=False)
    for _, grp in group_iter:
        idx = grp.index.to_numpy(dtype=int)
    ...
    keep = [c for c in [*_identity_columns(work), "wse_support_z_m", "support_distance_m", "support_type", "has_support", "wse_anchor_raw_z_m", "wse_interp_z_m", "wse_trend_z_m", "wse_pre_smooth_z_m", "wse_proxy_z_m", "final_adjustment_m", "final_adjustment_reason", "wse_valid", "wse_quality_flag"] if c in work.columns]
    ...
    summary = {
        **_direction_summary(final_gdf, "wse_proxy_z_m"),
        ...
        "support_source_direct_profile_count": int(direct_profile_fallback_count),
...
def _validate_wse_output(gdf: gpd.GeoDataFrame) -> dict[str, Any]:
    ...
    return {
        "valid": not missing and len(gdf) > 0 and finite == len(gdf) and monotone_violation_count == 0,
        "missing_required_fields": missing,
        "finite_wse_count": finite,
        "required_finite_wse_count": int(len(gdf)),
...
def run_wse_proxy_stage(
    ...
    *,
    ...
    aux_outputs = {}
    support_gdf = gpd.read_file(support_points_path)
    support_path = Path(support_points_path)
    support_summary = _summarize_support_points(support_gdf)
    ...
    diagnostics = {
        ...
        "source_logic": "river_v2_wse_proxy:explicit_support_to_trend_to_pre_smooth_to_final",
    ...
    receipt = build_river_v2_stage_receipt(
        ...
        input_artifacts=ctx.direct_stage_input_artifacts(support_points_path),
