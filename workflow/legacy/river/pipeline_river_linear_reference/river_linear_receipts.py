...
from pipeline.river_linear.river_linear_contract import LINEAR_STAGE_ORDER, diagnostic_contract_for_stage, validation_checks_for_stage
...
def build_stage_trace_entry(
    *,
    ...
    stage_order_index = LINEAR_STAGE_ORDER.index(stage_name)
    primary_output = _first_mapping_item(outputs)
    ...
    return {
        ...
        'receipt_path': str(receipt_path),
        'upstream_stage': LINEAR_STAGE_ORDER[stage_order_index - 1] if stage_order_index > 0 else None,
        'downstream_stage': LINEAR_STAGE_ORDER[stage_order_index + 1] if stage_order_index + 1 < len(LINEAR_STAGE_ORDER) else None,
