...
def _stage_receipt_paths(ctx: RiverLinearContext) -> dict[str, Path | None]:
    return {
...
def canonical_science_summary_from_stage_receipts(ctx: RiverLinearContext) -> dict[str, Any]:
...
def construction_stage_receipt_hashes(ctx: RiverLinearContext) -> dict[str, dict[str, Any]]:
...
def canonical_solution_manifest_path(ctx: RiverLinearContext) -> Path:
    return ctx.paths.manifests_dir / "canonical_river_solution_manifest.json"


def public_canonical_solution_manifest_path(ctx: RiverLinearContext) -> Path:
    # ctx.paths.root is <run>/river_linear in the active workflow.  The public
...
def canonical_solution_cache_manifest_path(ctx: RiverLinearContext) -> Path | None:
...
def write_canonical_solution_manifest(*, ctx: RiverLinearContext, payload: Mapping[str, Any]) -> tuple[Path, Path]:
