...
from pipeline.river_linear.river_linear_stage_backbone import BackboneStageResult
...
from pipeline.river_linear.river_linear_validation import validate_surface_raster
...
def _lateral_artifacts_dir(ctx: RiverLinearContext) -> Path:
...
def _write_surface_summary(ctx: RiverLinearContext, summary: dict[str, float | int | None]) -> None:
...
def _write_surface_contract_receipt(
    ...
    *,
    ...
    path.write_text(json.dumps(_jsonable(payload), indent=2, sort_keys=True) + '\n', encoding='utf-8')
    if failures:
        raise RuntimeError('river_linear_surface_contract_failed:' + ','.join(failures) + f':summary={path}')
    return path
...
def run_surface_stage(
    ...
    if backbone.empty:
        raise RuntimeError('river_linear_surface_missing_backbone')
    with rasterio.open(corridor_result.river_corridor_solve_path) as corridor_ds:
        corridor = corridor_ds.read(1) > 0
    if not np.any(corridor):
        raise RuntimeError('river_linear_surface_empty_corridor')
    ...
    if not np.any(seed_mask):
        raise RuntimeError('river_linear_surface_no_seed_pixels')
