...
from pipeline.river_linear.river_linear_execution_contract import CANONICAL_BUILD_ROLE, normalize_execution_role


@dataclass(frozen=True)
class RiverLinearSourceBundle:
...
class RiverLinearContext:
    ...
    def __post_init__(self, river_method: str | None) -> None:
        ...
        if not selected_name and str(river_method or '').strip():
            selected_name = str(river_method)
        ...
        if self.paths is None:
            object.__setattr__(self, "paths", build_river_linear_paths(self.out_dir, write_diagnostics=bool(self.write_diagnostics), write_core_receipts=bool(self.write_core_receipts)))
    ...
    @property
    def river_method(self) -> str:
        """Deprecated compatibility alias for the built-in workflow label."""
...
__all__ = ["CanonicalRiverSourceContext", "RiverLinearContext", "RiverLinearSourceBundle"]
