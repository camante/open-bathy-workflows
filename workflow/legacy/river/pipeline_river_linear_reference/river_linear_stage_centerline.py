...
from pipeline.river_linear.river_linear_validation import validate_centerline_points_gpkg, validate_centerline_points_overlap_template
from river_structured_scaffold import build_centerline_points
from pipeline.river_linear.river_linear_stage_helpers import attach_component_width_proxy, canonicalize_centerline_points, validate_centerline_points
...
def _read_required_layer(gpkg_path: Path, layer_name: str) -> gpd.GeoDataFrame:
    ...
    if layer_name not in names:
        raise RuntimeError(f'river_linear_missing_required_layer:{layer_name}')
    gdf = gpd.read_file(gpkg_path, layer=layer_name)
    if gdf is None or gdf.empty:
        raise RuntimeError(f'river_linear_empty_required_layer:{layer_name}')
...
def run_centerline_stage(
    ...
    *,
    ...
    if canonical_measured is None or not canonical_measured.exists():
        raise RuntimeError('river_linear_centerline_missing_canonical_measured_only')
    if Path(authoritative_result.solve_authoritative_base_measured_only_path).resolve() != canonical_measured.resolve():
        raise RuntimeError(
            f'river_linear_centerline_noncanonical_measured_source:{authoritative_result.solve_authoritative_base_measured_only_path}:{canonical_measured}'
        )
    ...
    if centerline is None or getattr(centerline, 'empty', True):
        raise RuntimeError('river_linear_centerline_no_points')
    ...
    if not validation.get('valid'):
        raise RuntimeError(f'river_linear_centerline_invalid:{validation}')
    ...
    min_export_points = int(getattr(getattr(ctx, 'cfg', None), 'river_linear_min_centerline_export_points', max(10, int(round(300.0 / max(float(spacing_m), 1.0))))))
