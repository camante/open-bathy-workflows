...
from pipeline.river_linear.river_linear_validation import validate_wse_proxy_points_gpkg
...
_WSE_PROFILE_CONTRACT_VERSION = 'monotone_longitudinal_profile_v4_clean_one_path'
...
def build_wse_support_artifact(out: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    """Return the explicit WSE support artifact consumed by the WSE trend step."""
...
def build_wse_trend_artifact(out: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    """Return the explicit WSE trend artifact consumed by pre-smoothing."""
    base_cols = _base_wse_columns(out)
...
def build_wse_pre_smooth_artifact(out: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    """Return the explicit pre-final-monotone WSE artifact."""
    base_cols = _base_wse_columns(out)
...
def build_wse_proxy_final_artifact(out: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    """Return the final WSE proxy artifact consumed by offset/backbone stages."""
    proxy = out.copy()
...
def _wse_stage_check_records(
    *,
    paths: dict[str, Path],
    science_summary: dict[str, Any],
    group_direction_checks: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    """Build the four-step WSE diagnostic contract without changing construction."""
...
def _write_wse_stage_contract(
    ...
    *,
    ...
    contract_path = _wse_artifact_reports_dir(ctx) / 'wse_one_path_stage_contract.json'
    checks = _wse_stage_check_records(
        paths=paths,
        science_summary=science_summary,
        group_direction_checks=group_direction_checks,
    )
    payload = {
        'schema': 'wse_one_path_stage_contract_v1',
        'stage': 'centerline_wse_proxy',
        'profile_contract_version': _WSE_PROFILE_CONTRACT_VERSION,
        'purpose': 'Expose WSE as four explicit diagnostic steps: support -> trend -> pre_smooth -> proxy_final.',
    ...
    lines = [
        ...
        f"profile_contract_version: {_WSE_PROFILE_CONTRACT_VERSION}",
...
def _wse_reports_dir(ctx: RiverLinearContext) -> Path:
    return ctx.paths.root.parent / 'reports'


def _wse_artifact_reports_dir(ctx: RiverLinearContext) -> Path:
...
def _write_wse_audit_artifacts(
    ...
    *,
    ...
    wse_stage_contract_path = _write_wse_stage_contract(
        ctx,
        paths=paths,
        science_summary=science_summary,
        group_direction_checks=group_direction_checks,
...
def run_wse_proxy_stage(
    ...
    if canonical_measured is None or not canonical_measured.exists():
        raise RuntimeError('river_linear_wse_missing_canonical_measured_only')
    if Path(authoritative_result.solve_authoritative_base_measured_only_path).resolve() != canonical_measured.resolve():
        raise RuntimeError(
            f'river_linear_wse_noncanonical_measured_source:{authoritative_result.solve_authoritative_base_measured_only_path}:{canonical_measured}'
        )
    ...
    if fit_failures:
        ...
        raise RuntimeError(
            'river_linear_wse_profile_fit_failed:'
    ...
    if finite == 0:
        raise RuntimeError('river_linear_wse_profile_all_nan')
