...
from pipeline.river_linear.river_linear_canonical_domain import (
...
from pipeline.river_linear.river_linear_receipts import write_canonical_identity_receipt
...
def _extract_linear_layers(source_gpkg: Path) -> tuple[gpd.GeoDataFrame, gpd.GeoDataFrame | None]:
    ...
    if flows is None or flows.empty:
        raise RuntimeError("river_linear_missing_canonical_flow_layer")
    polygons = None
...
def _prepared_solve_bundle(ctx: RiverLinearContext) -> dict[str, Any] | None:
...
def run_solve_domain_stage(ctx: RiverLinearContext) -> SolveDomainStageResult:
