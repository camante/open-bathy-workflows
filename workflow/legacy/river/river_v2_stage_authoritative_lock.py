...
from legacy.river.river_v2_contract import RiverV2StageResult, STAGE_RIVER_PRIMARY_SURFACE_LOCKED
from legacy.river.river_v2_receipts import build_river_v2_raster_receipt, write_river_v2_receipt
from legacy.river.river_v2_validation import ensure_existing_path, validate_authoritative_locked_raster
...
def build_authoritative_locked_primary_surface(
    ...
    *,
    ...
    authoritative_support_raster_path: Path | None = None,
    output_path: Path | None = None,
    receipt_path: Path | None = None,
    lock_domain_role: str = "solve_domain",
) -> RiverV2StageResult:
    ...
    auth_valid_base = np.isfinite(auth) & (auth != float(surf_nodata))
    auth_valid = auth_valid_base.copy()
    support_raster = Path(authoritative_support_raster_path) if authoritative_support_raster_path is not None else None
    support_valid = None
    lock_mask_source = "finite_authoritative_base"
    if support_raster is not None and support_raster.exists():
        support = _read_aligned(support_raster, out_shape=out_shape, out_transform=out_transform, out_crs=out_crs, nodata_value=float(surf_nodata))
        support = sanitize_array(support, float(surf_nodata), dtype="float32")
        support_valid = np.isfinite(support)
        auth_valid = auth_valid_base & support_valid
        if np.count_nonzero(auth_valid) == 0 and np.count_nonzero(auth_valid_base) > 0 and np.count_nonzero(support_valid) > 0:
            auth_valid = auth_valid_base
            lock_mask_source = "explicit_authoritative_support_raster_no_overlap_fallback_to_finite_authoritative_base"
        else:
            lock_mask_source = "explicit_authoritative_support_raster"

    write_nodata = float(surf_nodata)
    locked = surface.copy()
    ...
    locked[~np.isfinite(locked)] = write_nodata

    out_path = Path(output_path) if output_path is not None else ctx.paths.river_primary_surface_authoritative_applied_solve_domain
    ...
    profile.pop("BLOCKXSIZE", None)
    profile.pop("BLOCKYSIZE", None)
    profile.update(driver="GTiff", dtype="float32", count=1, nodata=write_nodata, compress="deflate", tiled=False)
    with rasterio.open(out_path, "w", **profile) as ds:
        ds.write(locked.astype("float32"), 1)
    with rasterio.open(diff_path, "w", **profile) as ds:
        ds.write(diff.astype("float32"), 1)

    validation = validate_authoritative_locked_raster(
        out_path,
        primary_surface_path,
        authoritative_base,
        authoritative_support_raster_path=support_raster,
    )
    ...
    receipt = build_river_v2_raster_receipt(
        ...
        input_artifacts=ctx.direct_stage_input_artifacts(primary_surface_path, authoritative_base, support_raster),
        ...
        stats={
            ...
            "lock_domain_role": str(lock_domain_role),
            "authoritative_base_path": str(authoritative_base),
            "authoritative_support_raster_path": str(support_raster) if support_raster is not None else None,
            "authoritative_support_locked_cell_count": int(np.count_nonzero(support_valid)) if support_valid is not None else None,
            "lock_mask_source": lock_mask_source,
            "output_path": str(out_path),
        },
    )
    receipt_path = write_river_v2_receipt(receipt, Path(receipt_path) if receipt_path is not None else ctx.paths.river_primary_surface_authoritative_applied_solve_domain_receipt)
