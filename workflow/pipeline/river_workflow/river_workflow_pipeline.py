...
from pipeline.river_workflow.river_workflow_execution_contract import AOI_EXPORT_ONLY_ROLE, CANONICAL_BUILD_ROLE, build_execution_contract, ensure_stage_allowed
...
from pipeline.river_workflow.river_workflow_contract import WORKFLOW_STAGE_ORDER, validate_run_contract
from pipeline.river_workflow.river_workflow_receipts import (
...
from pipeline.river_workflow.stage_03_authoritative_source import AuthoritativeStageResult, run_authoritative_stage
from pipeline.river_workflow.stage_06_authoritative_bed import AuthoritativeBedStageResult, run_authoritative_bed_stage
from pipeline.river_workflow.stage_09_bed_backbone import (
    BackboneStageResult,
    run_backbone_stage,
    validate_backbone_export_subset_flatness,
)
from pipeline.river_workflow.stage_04_centerline import CenterlineStageResult, run_centerline_stage
from pipeline.river_workflow.stage_10_river_corridor import CorridorStageResult, run_corridor_stage
from pipeline.river_workflow.stage_13_aoi_export import ExportStageResult, run_export_stage
from pipeline.river_workflow.stage_14_final_products import FinalDemStageResult, run_final_dem_stage
from pipeline.river_workflow.stage_02_grids import GridStageResult, run_grid_stage, _project_bounds, _write_subset_template_from_parent
from pipeline.river_workflow.stage_12_authoritative_lock import LockStageResult, run_lock_stage, write_authoritative_lock_contract_for_paths
from pipeline.river_workflow.stage_08_modeled_offset import ModeledOffsetStageResult, run_modeled_offset_stage
from pipeline.river_workflow.stage_07_observed_offset import ObservedOffsetStageResult, run_observed_offset_stage
from pipeline.river_workflow.stage_01_solve_domain import SolveDomainStageResult, run_solve_domain_stage
from pipeline.river_workflow.stage_11_primary_surface import SurfaceStageResult, run_surface_stage
from pipeline.river_workflow.stage_05_wse_proxy import WSEProxyStageResult, run_wse_proxy_stage
from pipeline.river_workflow.river_workflow_science_audit import write_river_science_chain_summary
from pipeline.river_workflow.river_workflow_export_logic import subset_float_raster_to_template_exact
from pipeline.river_workflow.river_workflow_identity import assert_parent_window_matches_export
from pipeline.river_workflow.river_workflow_canonical_manifest import (
...
class RiverWorkflowPipelineResult:
...
class CanonicalRiverBuildResult:
    ...
    result: RiverWorkflowPipelineResult
...
def _read_shared_solve_cache_key(ctx: RiverWorkflowContext) -> str | None:
...
def _existing_canonical_manifest_for_aoi_export(ctx: RiverWorkflowContext) -> Path | None:
...
def _bootstrap_cache_manifest_for_existing_parent(ctx: RiverWorkflowContext, parent_dem: Path | None) -> Path | None:
...
def _raw_existing_canonical_parent_path(ctx: RiverWorkflowContext) -> Path | None:
    bundle = ctx.linear_inputs
    if bundle is None:
        return None
...
def _existing_canonical_parent_for_aoi_export(ctx: RiverWorkflowContext) -> Path | None:
...
def _load_export_canonical_manifest(ctx: RiverWorkflowContext) -> tuple[Path, dict[str, Any]]:
...
def _window_dict_from_parent_and_template(parent_path: Path, template_path: Path) -> dict[str, int] | None:
    ...
    except (OSError, ValueError, TypeError, _rio.errors.RasterioError):
        return None


def _assert_export_only_result_has_no_construction_stage_receipts(result: RiverWorkflowPipelineResult) -> None:
...
def run_aoi_export_from_canonical_parent(ctx: RiverWorkflowContext, handoff: CanonicalParentHandoff | Path | None = None) -> RiverWorkflowPipelineResult:
    logger = logging.getLogger(getattr(ctx, "logger_name", "river_workflow"))
    ...
    export_only_stage_receipts = {"aoi_export_only": ctx.paths.aoi_export_receipt}
    execution_mode_receipt = write_execution_mode_receipt(
        out_dir=ctx.paths.root.parent,
        execution_role=AOI_EXPORT_ONLY_ROLE,
        effective_mode="aoi_export_only",
        stage_receipts=export_only_stage_receipts,
        canonical_parent_dem=parent_for_receipts,
        aoi_export_dem=ctx.paths.aoi_export_dem,
        final_user_dem=None,
        canonical_manifest_path=public_canonical_solution_manifest_path(ctx),
        source="aoi_export_from_canonical_parent",
    )
    export_only_stage_receipts["river_execution_mode"] = execution_mode_receipt.receipt_path
    write_receipt_purpose_manifest(
    ...
    result = RiverWorkflowPipelineResult(ctx.paths.run_contract, True, solve_result, grid_result, authoritative_result, centerline_result, wse_result, authoritative_bed_result, observed_offset_result, modeled_offset_result, backbone_result, corridor_result, surface_result, lock_result, export_result, final_dem_result, export_only_stage_receipts, None, None, None, None, None, None, None, None, None)
    _assert_export_only_result_has_no_construction_stage_receipts(result)
    return result

def run_canonical_river_build(ctx: RiverWorkflowContext) -> RiverWorkflowPipelineResult:
    ...
    if ctx.write_diagnostics or getattr(ctx, "write_core_receipts", True):
        ...
        execution_mode_receipt = write_execution_mode_receipt(
            ...
            canonical_parent_dem=final_dem_result.canonical_parent_dem_path,
            aoi_export_dem=final_dem_result.aoi_export_dem_path,
            final_user_dem=final_dem_result.dem_enhanced_final_path,
            canonical_manifest_path=public_canonical_solution_manifest_path(ctx),
            source="canonical_river_build",
        )
        receipts["river_execution_mode"] = execution_mode_receipt.receipt_path
        write_receipt_purpose_manifest(
            ctx.paths.receipt_purpose_manifest,
            ctx=ctx,
            stage_receipts=receipts,
    ...
    return RiverWorkflowPipelineResult(
...
def resolve_canonical_parent_handoff(ctx: RiverWorkflowContext) -> CanonicalParentHandoff | None:
...
def run_river_parent_export_workflow(ctx: RiverWorkflowContext) -> RiverWorkflowPipelineResult:
...
def run_river_workflow_pipeline(ctx: RiverWorkflowContext) -> RiverWorkflowPipelineResult:
