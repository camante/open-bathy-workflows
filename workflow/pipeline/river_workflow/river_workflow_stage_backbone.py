...
from pipeline.river_workflow.river_workflow_stage_modeled_offset import ModeledOffsetStageResult
from pipeline.river_workflow.river_workflow_stage_wse_proxy import WSEProxyStageResult
from pipeline.river_workflow.river_workflow_validation import validate_backbone_points_gpkg
...
def _points_in_export_grid(frame: gpd.GeoDataFrame, export_grid_template: Path) -> gpd.GeoDataFrame:
    ...
    if not export_grid_template.exists():
        raise RuntimeError(f'river_workflow_backbone_export_grid_missing:{export_grid_template}')
    ...
    if work.crs is None and grid_crs is not None:
        raise RuntimeError('river_workflow_backbone_export_subset_crs_missing')
...
def validate_backbone_export_subset_flatness(
    ...
    if not path.exists():
        raise RuntimeError(f'river_workflow_backbone_points_missing:{path}')
    ...
    if export_subset_check.get('fail_reason'):
        ...
        raise RuntimeError(
            'river_workflow_backbone_export_subset_flatness_fail:'
...
def _oriented_downstream_station(station_m: Any, wse_proxy_z_m: Any | None = None) -> tuple[np.ndarray, int, float | None]:
    ...
    if wse_proxy_z_m is not None:
        ...
        if np.count_nonzero(mask) >= 3 and float(np.nanmax(sta[mask]) - np.nanmin(sta[mask])) > 0.0:
            ...
            except (np.linalg.LinAlgError, TypeError, ValueError, FloatingPointError):
                slope = None
...
def _bounded_residual_preserving_smooth(
    *,
    ...
    if (
        ...
        raise RuntimeError(
            'river_workflow_backbone_transform_flattened_raw_signal:'
...
def run_backbone_stage(
    ...
    if 'point_id' not in wse.columns or 'point_id' not in modeled.columns:
        raise RuntimeError('river_workflow_backbone_missing_point_id')
    ...
    if merged.empty:
        raise RuntimeError('river_workflow_backbone_empty_join')
    ...
    if formula_failures:
        ...
        raise RuntimeError(
            'river_workflow_backbone_formula_handoff_fail:'
