...
from pipeline.river_workflow.river_workflow_validation import validate_authoritative_bed_points_gpkg
from pipeline.river_workflow.river_workflow_stage_helpers import sample_raster_to_points, validate_centerline_authoritative_bed
...
def _require_canonical_authoritative_inputs(
    ...
    if not expected_measured.exists():
        raise RuntimeError(f'river_workflow_missing_canonical_measured_only:{expected_measured}')
    if not expected_support.exists():
        raise RuntimeError(f'river_workflow_missing_canonical_support_mask:{expected_support}')
    ...
    if actual_measured.resolve() != expected_measured.resolve():
        raise RuntimeError(f'river_workflow_authoritative_bed_noncanonical_measured_source:{actual_measured}:{expected_measured}')
    if actual_support.resolve() != expected_support.resolve():
        raise RuntimeError(f'river_workflow_authoritative_bed_noncanonical_support_source:{actual_support}:{expected_support}')
...
def run_authoritative_bed_stage(
    ...
    if not validation.get('valid'):
        raise RuntimeError(f"river_workflow_authoritative_bed_invalid:{validation}:diag={diagnostics}")
