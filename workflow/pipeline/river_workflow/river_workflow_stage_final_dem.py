...
from pipeline.river_workflow.river_workflow_final_dem_logic import FinalDemCompositionSummary, compose_final_dem
from pipeline.river_workflow.river_workflow_receipts import write_aoi_export_dem_receipt, write_canonical_parent_dem_receipt
...
from pipeline.river_workflow.river_workflow_stage_export import ExportStageResult
from pipeline.river_workflow.river_workflow_stage_lock import LockStageResult
from pipeline.river_workflow.river_workflow_stage_grids import GridStageResult
from pipeline.river_workflow.river_workflow_export_logic import build_take_mask, subset_float_raster_to_template_exact
from pipeline.river_workflow.river_workflow_identity import assert_parent_window_matches_export
from pipeline.river_workflow.river_workflow_validation import validate_final_dem_raster, validate_raster_matches_template
from pipeline.river_workflow.river_workflow_cache import build_canonical_cache_metadata, validate_cached_parent_dem
...
def _template_window_identity(parent_dem: Path, export_template: Path) -> dict[str, Any]:
    ...
    with rasterio.open(parent_dem) as parent, rasterio.open(export_template) as tmpl:
        ...
        except (TypeError, ValueError, OverflowError, rasterio.errors.RasterioError) as exc:  # diagnostic only; do not hide export failures elsewhere
            return {
                'parent_path': str(parent_dem),
                'export_template_path': str(export_template),
                'export_bounds': [float(tmpl.bounds.left), float(tmpl.bounds.bottom), float(tmpl.bounds.right), float(tmpl.bounds.top)],
