...
from core.nodata_utils import fill_output_nodata, sanitize_for_output, array_valid_mask, collect_array_nodata_audit
...
from pipeline.final_route.final_route_contract import validate_final_route_contract
...
def _validate_written_authoritative_lock_contract(*, final_path: Path, aligned_auth_path: Path, support_path: Path, tolerance: float = 1.0e-6) -> dict:
    from pipeline.final_dem.final_dem_contract_validator import validate_written_final_dem_contract
...
def write_final_route_outputs(*, cfg, paths, guidance, terrain, candidate_path: Path | None, provenance_path: Path | None, report: dict) -> tuple[Path, Path, Path, Path, Path, Path]:
    ...
    log.info("[FINAL_ROUTE][DEBUG] Final-route stage debug directory: %s", debug_dir)
    log.info("[FINAL_ROUTE][DEBUG] Final-route stage debug manifest: %s", stage_debug_manifest_path)
