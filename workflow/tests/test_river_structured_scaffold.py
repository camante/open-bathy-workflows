...
def test_structured_scaffold_points_are_polygon_bounded(tmp_path: Path):
    ...
    xs.to_file(xs_gpkg, driver="GPKG")
...
def test_dense_bank_points_can_use_xs_fallback_and_along_bank_fill(tmp_path: Path):
    poly = gpd.GeoDataFrame({"id": [1]}, geometry=[Polygon([(2, 2), (18, 2), (18, 18), (2, 18)])], crs="EPSG:32619")
    arr = np.ones((20, 20), dtype=np.float32) * -9999.0
    tif = tmp_path / "bed2.tif"
    _write_test_raster(tif, arr, from_origin(0, 20, 1, 1))
    xs_bank = gpd.GeoDataFrame(
        {"bank_z_m": [3.5, 3.7]},
        geometry=[Point(3.0, 3.0), Point(17.0, 17.0)],
        crs="EPSG:32619",
    )

    bank = build_dense_bank_points(poly, spacing_m=6.0, raster_path=tif, normal_search_max_m=1.5, normal_search_step_m=1.0, xs_bank_points=xs_bank)
    assert not bank.empty
    assert np.isfinite(bank["bank_z_m"]).all()
    assert set(bank["bank_sample_status"]).intersection({"xs_fallback", "along_bank_interp"})
