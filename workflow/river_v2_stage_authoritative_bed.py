...
from river_v2_contract import RiverV2StageResult, STAGE_CENTERLINE_AUTHORITATIVE_BED
...
def sample_authoritative_bed_to_centerline(ctx: RiverV2Context, centerline_points_gdf: gpd.GeoDataFrame) -> tuple[gpd.GeoDataFrame, list[str], str | None, dict[str, Any]]:
    ...
    if np.count_nonzero(finite_mask) == 0:
        ...
        if trusted_mode == "metadata_proven_only" and "authoritative_bed_zero_valid_points_low_support_mode" not in warnings:
            warnings.append("authoritative_bed_zero_valid_points_low_support_mode")
        elif trusted_mode == "all_finite_cells_trusted" and "authoritative_bed_zero_valid_points_low_support_mode" not in warnings:
            warnings.append("authoritative_bed_zero_valid_points_low_support_mode")
    keep = [c for c in (
...
def run_authoritative_bed_stage(ctx: RiverV2Context, *, centerline_points_path: Path, authoritative_support_points_path: Path | None) -> RiverV2StageResult:
    centerline = _load_centerline_points(centerline_points_path)
    authoritative_source = Path(authoritative_support_points_path) if authoritative_support_points_path is not None else None
    gdf, warnings, method, diagnostics = sample_authoritative_bed_to_centerline(ctx, centerline)
    ...
    receipt = build_river_v2_stage_receipt(
        ...
        input_artifacts=ctx.direct_stage_input_artifacts(
            ...
            ctx.authoritative_sampling_raster_path,
        ...
        warnings=warnings,
        source_logic=f"centerline_authoritative_bed_native_v2:{method or 'unknown'}:projected_authoritative_sampling_raster",
    ...
    return RiverV2StageResult(
        ...
        validation=validation,
        warnings=warnings,
        aux_outputs={"authoritative_source": str(authoritative_source)} if authoritative_source is not None else {},
    )
