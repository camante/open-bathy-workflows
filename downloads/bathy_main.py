...
class BathyConfig:
    ...
    river_thalweg_only: bool = False  # if True, pass --thalweg-only to xs_infer_bathy_raster.py
...
def run_river(cfg: BathyConfig, report: Dict[str, Any]) -> Optional[Path]:
    ...
    if bool(getattr(cfg, 'river_thalweg_only', False)):
        cmd.append('--thalweg-only')
    cmd += [
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--river-thalweg-only", action="store_true", default=False,
                   help="Use thalweg-only control points (one per cross-section) when rasterizing river bed; improves longitudinal continuity in sparse data.")
