...
def _pick_first_numeric_col(df: pd.DataFrame, candidates: List[str]) -> Optional[str]:
    for c in candidates:
        ...
        if np.isfinite(s.to_numpy(dtype="float64")).any():
            return c
    return None


def _xs_overlap_score_series(xs_lines_m: gpd.GeoDataFrame, mode: str = "auto") -> pd.Series:
    """Return a priority score per XS (higher = kept) for overlap filtering.

    We use a greedy selection that keeps higher-scored transects when transects
    intersect in their interiors (a common artifact near confluences / multi-scale
    segmenting). 'auto' prefers drainage area → width → stream order → length.
    """
    mode = str(mode).strip().lower()

    width_cols = ["width_m", "bankfull_w_m", "bf_width_m", "w_m", "width"]
    da_cols = ["drain_area_km2", "da_km2", "drainarea_km2", "drain_area", "area_km2"]
    order_cols = ["stream_order", "strahler", "order", "str_order"]

    if mode == "auto":
        col = _pick_first_numeric_col(xs_lines_m, da_cols) or _pick_first_numeric_col(xs_lines_m, width_cols) or _pick_first_numeric_col(xs_lines_m, order_cols)
        if col is not None:
            s = pd.to_numeric(xs_lines_m[col], errors="coerce")
        else:
            s = pd.to_numeric(xs_lines_m.get("_xs_len_m", xs_lines_m.geometry.length), errors="coerce")
    elif mode in ("drain_area", "da", "drainage"):
        col = _pick_first_numeric_col(xs_lines_m, da_cols)
        s = pd.to_numeric(xs_lines_m[col], errors="coerce") if col else pd.Series(np.nan, index=xs_lines_m.index)
    elif mode == "width":
        col = _pick_first_numeric_col(xs_lines_m, width_cols)
        s = pd.to_numeric(xs_lines_m[col], errors="coerce") if col else pd.Series(np.nan, index=xs_lines_m.index)
    elif mode in ("order", "stream_order"):
        col = _pick_first_numeric_col(xs_lines_m, order_cols)
        s = pd.to_numeric(xs_lines_m[col], errors="coerce") if col else pd.Series(np.nan, index=xs_lines_m.index)
    else:  # length
        s = pd.to_numeric(xs_lines_m.get("_xs_len_m", xs_lines_m.geometry.length), errors="coerce")
    ...
    # Keep non-negative (defensive)
    s = s.clip(lower=0.0)
    return s


def _as_linestring_for_intersection(g):
    """Best-effort conversion to a LineString for intersection tests."""
    if g is None:
        return None
    try:
        if g.is_empty:
    ...
    if gt == "MultiLineString":
        try:
            m = linemerge(g)
            if getattr(m, "geom_type", None) == "LineString":
                return m
            if getattr(m, "geom_type", None) == "MultiLineString":
                parts = list(getattr(m, "geoms", []))
                if parts:
                    parts.sort(key=lambda gg: gg.length, reverse=True)
                    return parts[0]
        except Exception:
            parts = list(getattr(g, "geoms", []))
            if parts:
                parts.sort(key=lambda gg: gg.length, reverse=True)
                return parts[0]
        return None

    # As a fallback for odd geometries, try the boundary
    try:
        b = g.boundary
        if getattr(b, "geom_type", None) == "LineString":
            return b
        if getattr(b, "geom_type", None) == "MultiLineString":
            m = linemerge(b)
            if getattr(m, "geom_type", None) == "LineString":
                return m
    except Exception:
...
def _bad_xs_intersection(li, lj, endpoint_tol_m: float = 2.0) -> bool:
    """Return True if two XS lines intersect in their interiors (not just at ends)."""
    li = _as_linestring_for_intersection(li)
    lj = _as_linestring_for_intersection(lj)
    ...
    try:
        ...
        # If robust predicate fails, assume no
        return False
    ...
    if gt in ("LineString", "MultiLineString"):
        # Overlapping segments are always bad
        return True

    # Treat point-ish intersections as bad only if they occur away from both endpoints
    try:
        p = inter.centroid
    except Exception:
        return True
    ...
    try:
        len_i = float(li.length)
        len_j = float(lj.length)
    except Exception:
        return True

    if not np.isfinite(len_i) or not np.isfinite(len_j) or len_i <= 0 or len_j <= 0:
        return True

    # Scale tolerance by line length (2% or absolute meters, whichever is larger)
    tol = float(max(endpoint_tol_m, 0.02 * min(len_i, len_j)))

    try:
        si = float(li.project(p))
        sj = float(lj.project(p))
    except Exception:
        return True

    interior_i = (si > tol) and (si < (len_i - tol))
    interior_j = (sj > tol) and (sj < (len_j - tol))
    return bool(interior_i and interior_j)


def _filter_xs_overlaps(
    xs_lines: gpd.GeoDataFrame,
    xs_pts: gpd.GeoDataFrame,
    enabled: bool = True,
    score_mode: str = "auto",
    endpoint_tol_m: float = 2.0,
) -> Tuple[gpd.GeoDataFrame, gpd.GeoDataFrame, int]:
    """Drop intersecting XS transects (a common source of raster 'blobs' and checkerboard artifacts).

    Strategy:
      - Project to a metric CRS (UTM) for robust intersection tests.
      - Score each transect (auto: DA→width→order→length).
      - Greedily keep higher-scored transects, skipping any that intersect an already-kept transect
        in the *interior* (endpoint-only touches are ignored).
      - Filter xs_points to match.

    Returns: (xs_lines_filtered, xs_pts_filtered, n_removed)
    """
    if (not enabled) or xs_lines is None or xs_lines.empty:
        return xs_lines, xs_pts, 0
    ...
    # Work in a projected CRS for intersection tolerances in meters
    xs_lines_m = xs_lines
    try:
        crs0 = CRS.from_user_input(xs_lines.crs)
        if not crs0.is_projected:
            u = unary_union(list(xs_lines.geometry))
            c = u.centroid
            utm = _utm_crs_from_lonlat(float(c.x), float(c.y))
            xs_lines_m = xs_lines.to_crs(utm)
        else:
            xs_lines_m = xs_lines
    except Exception:
        xs_lines_m = xs_lines

    xs_lines_m = xs_lines_m.copy()
    xs_lines_m["_xs_len_m"] = xs_lines_m.geometry.length
    scores = _xs_overlap_score_series(xs_lines_m, mode=score_mode)

    # Greedy keep: highest score first
    order = np.argsort(-scores.to_numpy(dtype="float64"))
    n = len(xs_lines_m)
    kept = np.zeros((n,), dtype=bool)

    try:
        sindex = xs_lines_m.sindex
    except Exception:
        sindex = None

    geoms = list(xs_lines_m.geometry)

    for i in order.tolist():
        gi = _as_linestring_for_intersection(geoms[i])
        if gi is None:
            continue

        # Candidate overlaps: bbox query if available, else brute-force
        ...
        for j in cand:
            if j == i or (not kept[j]):
                continue
            gj = _as_linestring_for_intersection(geoms[j])
            if gj is None:
                continue
            if _bad_xs_intersection(gi, gj, endpoint_tol_m=float(endpoint_tol_m)):
                conflict = True
                break
    ...
    keep_ids = set(xs_lines.iloc[np.where(kept)[0]]["xs_id"].astype(str).tolist())
    xs_lines_f = xs_lines[xs_lines["xs_id"].astype(str).isin(keep_ids)].copy()
    xs_pts_f = xs_pts[xs_pts["xs_id"].astype(str).isin(keep_ids)].copy()
    removed_n = int(len(xs_lines) - len(xs_lines_f))
    return xs_lines_f, xs_pts_f, removed_n
...
def infer_bathy(
    ...
    channel_mask_inside_value: int = 1,
    channel_mask_invert: bool = False,
    # Cross-section overlap filtering (recommended; eliminates intersecting transects from mixed-scale networks)
    xs_overlap_filter: bool = True,
    xs_overlap_score: str = "auto",
    xs_overlap_endpoint_tol_m: float = 2.0,
) -> None:
    ...
    xs_lines = xs_lines.copy()
    xs_pts = xs_pts.copy()
    if bool(xs_overlap_filter):
        xs_lines, xs_pts, n_removed = _filter_xs_overlaps(
            xs_lines, xs_pts, enabled=True, score_mode=str(xs_overlap_score), endpoint_tol_m=float(xs_overlap_endpoint_tol_m)
        )
        if n_removed > 0:
            log.info(
                "[XS] Dropped %d intersecting cross-sections (score=%s, endpoint_tol_m=%.2f).",
                int(n_removed),
                str(xs_overlap_score),
                float(xs_overlap_endpoint_tol_m),
            )
...
def _parse_args() -> argparse.Namespace:
    ...
    # XS overlap filter (prevents intersecting cross-sections from mixed-scale networks)
    p.add_argument(
        "--disable-xs-overlap-filter",
        action="store_true",
        help=(
            "Disable filtering of intersecting cross-sections. By default, the script greedily "
            "keeps higher-priority transects and drops lower-priority transects whose lines intersect "
            "in their interiors (a common source of raster artifacts when cross-sections are generated "
            "from overlapping mainstem/tributary segments)."
        ),
    )
    p.add_argument(
        "--xs-overlap-score",
        choices=["auto", "drain_area", "width", "order", "length"],
        default="auto",
        help="Priority metric used when dropping overlapping cross-sections (higher is kept). 'auto' prefers drainage area → width → order → length.",
    )
    p.add_argument(
        "--xs-overlap-endpoint-tol-m",
        type=float,
        default=2.0,
        help="Ignore intersections that occur within this distance (m) of a transect end (reduces false positives from endpoint touches).",
    )
...
def main() -> None:
    ...
    infer_bathy(
        ...
        channel_mask_inside_value=int(args.channel_mask_inside_value),
        channel_mask_invert=bool(args.channel_mask_invert),
        xs_overlap_filter=not bool(args.disable_xs_overlap_filter),
        xs_overlap_score=str(args.xs_overlap_score),
        xs_overlap_endpoint_tol_m=float(args.xs_overlap_endpoint_tol_m),
    )
